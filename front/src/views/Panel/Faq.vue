<template>
  <div class="mt-10 px-8">
    <div class="flex justify-center">
      <span class="!font-medium text-lg">لیست سوالات متداول</span>
    </div>
    <div class="mt-10 px-3 mb-3">
      <div class="mb-3 mr-2 flex justify-between fm:flex-col items-center">
        <div class="flex gap-2 items-center">
          <UserCanAction action="create" @create="create()" permission="create_faqs" />
      ...
      <div v-if="allData.length > 0">
        <table class="w-full border-collapse lg:table 2xl:table xl:lg:table md:table">
          ...
          <tr v-for="item in allData" :key="item.id" class="border border-gray-200 block lg:table-row xl:table-row 2xl:table-row md:table-row">
            ...
            <Td title="دسته" class="fm:text-sm">{{item.category.title}}</Td>
            <Td title="عملیات">
              <div class="flex items-center justify-center gap-2">
                <UserCanAction action="show" @show="show(item.id)" permission="view_faqs" :postId="item.id" />
                <UserCanAction action="destroy" @destroy="destroy(item.id)" permission="update_faqs" :postId="item.id" />
              </div>
            </Td>
          </tr>
          </tbody>
    ...
    <Modal :title="isUpdate ? 'ویرایش سوالات متداول' : 'ثبت سوالات متداول'" :save="isUpdate ? 'ثبت تغییرات' : 'ثبت'" :permission="isUpdate ? 'update_faqs' : 'create_faqs'" :btnLoading="btnLoading" @callback="isUpdate ? update() : insert()" ref="openModal">
      <div class="mb-5">
      ...
      <div class="mb-5">
        <Textarea label="توضیحات" :cols="5" :rows="5" :maxlength="5000" :alert="description.length + '/5000'" v-model="description" />
      </div>
    </Modal>
    <Meta :title="$store.state.siteName + ` | لیست سوالات متداول `"/>
...
async function getData(_loading = true,page=1, isRefresh=false){
  ...
  await axios.get(`${store.state.api}${model.value}?page=${page}&search=${search.value}`).then(resp=>{
    ...
    allData.value = data.faqs.data;
    categories.value = data.categories;
    store.commit('paginate',data.faqs);
  }).catch(err=>{
