    ...
    <div class="fm:w-[990px] flex flex-col" id="print" v-if="order !== null">
      <div class="flex flex-col gap-3 fm:w-[990px]">
        <div class="flex items-center justify-between gap-2">
          ...
          <div class="flex flex-col gap-8 border border-gray-200 p-4 w-[77%]" v-if="seller !== null">
            <div>
              <ul class="flex justify-between">
                <li class="flex items-center gap-2">
                  <span class="!font-medium text-sm">فروشنده:</span>
                  <span class="text-sm">{{seller.company_name}} ({{seller.ir_company_type}})</span>
                </li>
                <li class="flex items-center gap-2">
                  <span class="!font-medium text-sm">شناسه ملی:</span>
                  <span class="text-sm">{{seller.national_identity_number}}</span>
                </li>
                <li class="flex items-center gap-2">
                  <span class="!font-medium text-sm">شماره ثبت:</span>
                  <span class="text-sm">{{seller.registration_number}}</span>
                </li>
                <li class="flex items-center gap-2">
                  <span class="!font-medium text-sm">شماره اقتصادی:</span>
                  <span class="text-sm">{{seller.economic_number}}</span>
                </li>
              </ul>
            </div>
            <div>
              <ul class="flex justify-between">
                <li class="flex items-center gap-2">
                  ...
                  <span class="text-sm">{{seller.address}}</span>
                </li>
                <li class="flex items-center gap-2">
                  <span class="!font-medium text-sm">کدپستی:</span>
                  <span class="text-sm">{{seller.postal_code}}</span>
                </li>
                <li class="flex items-center gap-2">
                  <span class="!font-medium text-sm">تلفن :</span>
                  <span class="text-sm">{{seller.phone}}</span>
                </li>
                <li class="flex items-center gap-2">
                  <span class="!font-medium text-sm">فکس:</span>
                  <span class="text-sm">{{seller.fax}}</span>
                </li>
              </ul>
            </div>
          </div>
                ...
                <table class="min-w-full border text-center text-sm font-light fm:w-[990px]">
                  ...
                  <tbody>
                    <tr class="border-b" v-for="(detail,index) in order.order_details" :key="detail.id">
                      ...
                      <td class="whitespace-nowrap border-r px-6 py-4">پرنساشاپ</td>
                      ...
                      <td class="whitespace-nowrap border-r px-6 py-4">{{$store.getters.numberFormat(detail.discount)}}</td>
                    ...
                    <tr class="border-b">
                      ...
                      <td class="whitespace-nowrap border-r px-6 py-4">{{$store.getters.numberFormat(order.order_details.reduce((n, {discount}) => n + discount, 0))}}</td>
                    ...
                    <tr class="border-b">
                      <td colspan="5" class="whitespace-nowrap border-r px-6 py-4"></td>
                      <td colspan="4" class="whitespace-nowrap border-r px-6 py-4">جمع کل پس از تخفیف + هزینه ارسال (ریال):</td>
                      <td class="whitespace-nowrap border-r px-6 py-4">{{$store.getters.numberFormat(((order.order_details.reduce((n, {amount}) => n + amount, 0))+parseInt(order.transport_cost)) - parseInt(order.reduced_wallet))}}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
      ...
      <div class="flex flex-col gap-10 fm:w-[990px] border border-gray-200 mb-10 p-4">
        <ul class="flex justify-between">
          <li class="flex flex-col gap-8">
            ...
            <img :src="seller.signature" class="w-[120px] h-[120px]" />
          </li>
          <li class="flex flex-col gap-8">
