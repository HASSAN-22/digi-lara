<template>
  <div class="mt-10 px-8">
    <div class="flex justify-center">
      <span class="!font-medium text-lg">لیست گزارشات کالا</span>
    </div>
      ...
      <div v-if="allData.length > 0">
        <table class="w-full border-collapse lg:table 2xl:table xl:lg:table md:table">
          ...
          <tr v-for="item in allData" :key="item.id" class="border border-gray-200 block lg:table-row xl:table-row 2xl:table-row md:table-row">
            ...
            <Td title="برای کالا" class="fm:text-sm">{{item.reportable.ir_name}}</Td>
    ...
    <Meta :title="$store.state.siteName + ` | لیست  گزارشات کالا`"/>
...
async function getData(_loading = true,page=1, isRefresh=false){
  ...
  await axios.get(`${store.state.api}product-reports?page=${page}&search=${search.value}`).then(resp=>{
