<template>
  <div class="mt-10 px-8">
    <div class="flex justify-center">
      <span class="!font-medium text-lg">لیست ویژگی ها</span>
    </div>
    <div class="mt-10 px-3 mb-3">
      <div class="mb-3 mr-2 flex justify-between fm:flex-col items-center">
        <div class="flex gap-2 items-center">
          <UserCanAction action="create" @create="create()" permission="create_properties" />
      ...
      <div v-if="allData.length > 0">
        <table class="w-full border-collapse lg:table 2xl:table xl:lg:table md:table">
          ...
          <tr v-for="item in allData" :key="item.id" class="border border-gray-200 block lg:table-row xl:table-row 2xl:table-row md:table-row">
            <Td title="عنوان" class="fm:text-sm">{{item.property}}</Td>
            <Td title="عملیات">
              <div class="flex items-center justify-center gap-2">
                <UserCanAction action="show" @show="show(item.id)" permission="view_properties" :postId="item.id" />
                <UserCanAction action="destroy" @destroy="destroy(item.id)" permission="update_properties" :postId="item.id" />
              </div>
            </Td>
          </tr>
          </tbody>
    ...
    <Modal :title="isUpdate ? 'ویرایش ویژگی' : 'ثبت ویژگی'" :save="isUpdate ? 'ثبت تغییرات' : 'ثبت'" :permission="isUpdate ? 'update_properties' : 'create_properties'" :btnLoading="btnLoading" @callback="isUpdate ? update() : insert()" ref="openModal">
      <Input label="ویژگی" placeholder="ویژگی" v-model="property" id="property" />
      <div class="mt-5">
        <Input label="مقادیر ویژگی" placeholder="مقدار را وارد کرده و اینتر برنید" alert="مقدار را وارد کرده و اینتر برنید" @keyup.enter="addPropertyType($event)" id="propertyType" />
        <div class="mt-4 flex flex-wrap gap-2 items-center">
          <div class="flex items-center gap-1 bg-gray-200 rounded-lg px-2 py-1" v-for="(type,index) in propertyTypes" :key="index">
            <span class="text-md">{{type}}</span>
            <span class="cursor-pointer fm:text-sm" @click="removePropertyType(index)"><i class="far text-sm fa-trash text-red-500"></i></span>
          </div>
        </div>
      </div>
    </Modal>
...
function addPropertyType(event){
  event.target.value.length > 0 ? propertyTypes.value.push(event.target.value) : '';
  event.target.value = null
}

function removePropertyType(index){
  propertyTypes.value = propertyTypes.value.filter((v,k)=>k !== index)
}
...
async function show(_postId){
  ...
  await axios.get(`${store.state.api}${model.value}/${postId.value}`).then(async resp=>{
    ...
    propertyTypes.value = data.property_types.map(item=>item.name);
  }).catch(err=>{
    store.commit('handleError',err)
  })
  loading.value = false;
...
function formData(){
  return {
    ...
    property__types:propertyTypes.value
  };
}
