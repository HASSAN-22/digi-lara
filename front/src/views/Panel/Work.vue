<template>
  <div class="mt-10 px-8">
    <div class="flex justify-center">
      <span class="!font-medium text-lg">لیست شغل ها</span>
    </div>
    <div class="mt-10 px-3 mb-3">
      <div class="mb-3 mr-2 flex justify-between fm:flex-col items-center">
        <div class="flex gap-2 items-center">
          <UserCanAction action="create" @create="create()" permission="create_works" />
      ...
      <div v-if="allData.length > 0">
        <table class="w-full border-collapse lg:table 2xl:table xl:lg:table md:table">
          ...
          <tr v-for="item in allData" :key="item.id" class="border border-gray-200 block lg:table-row xl:table-row 2xl:table-row md:table-row">
            <Td title="شغل" class="fm:text-sm">{{item.name}}</Td>
            <Td title="عملیات">
              <div class="flex items-center justify-center gap-2">
                <UserCanAction action="show" @show="show(item.id)" permission="view_works" :postId="item.id" />
                <UserCanAction action="destroy" @destroy="destroy(item.id)" permission="update_works" :postId="item.id" />
              </div>
            </Td>
          </tr>
          </tbody>
    ...
    <Modal :title="isUpdate ? 'ویرایش شغل' : 'ثبت شغل'" :save="isUpdate ? 'ثبت تغییرات' : 'ثبت'" :permission="isUpdate ? 'update_works' : 'create_works'" :btnLoading="btnLoading" @callback="isUpdate ? update() : insert()" ref="openModal">
      <Input label="شغل" placeholder="شغل" v-model="work" id="work" />
    </Modal>
    <Meta :title="$store.state.siteName + ` | لیست شغل ها `"/>
